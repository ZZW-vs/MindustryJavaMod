...
/**
 * LightningBurstAbility (PU132 unity.entities.abilities.LightningBurstAbility 完整移植)
 *
 * 功能: 单位落地时释放闪电爆发, 按 rechargeTime 冷却
 * - 落地时自动触发 (able = !isFlying)
 * - 释放 bolts 道闪电, 每道随机方向, 有 maxDelay 延迟
 * - 充能完成时播放 ringFx 环形特效
 * - 充能中播放 waitFx 进度弧线特效
 */
    ...
    public void used(Unit u){
        ...
        for(var i = 0; i < bolts; i++){
            ...
            Time.run(Mathf.random(maxDelay), () -> {
              ...
              Sounds.shootArc.at(u.x, u.y, 1.25f, 0.75f);  // v155.4: spark -> shootArc
            });
        }
    }
