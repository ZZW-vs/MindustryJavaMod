...
/**
 * 地板抽取器 (PU132 unity.world.blocks.production.FloorExtractor 移植)
 * <p>从地板/覆盖物类型抽取资源的工厂。根据方块下方地板的类型决定产出效率。
 * 直接从 PU132 移植, 仅修改包名, 无 @Merge 依赖。</p>
 *
 * @author GlennFolker (原 PU132 作者)
 */
public class FloorExtractor extends GenericCrafter{
    private static final Seq<Tile> source = new Seq<>();

    /** 地板/覆盖物 ID → 效率映射 (使用 block.id 作为 key) */
    ...
    /** 设置地板映射: [Block, float, Block, float, ...] */
