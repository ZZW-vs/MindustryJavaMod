...
import zzw.content.blocks.turrets.BoostLaserTurret;
...
public class Z_Turrets {
    ...
    /**
     * 全量加载 (兼容入口): 按 PU132 原版 UnityBlocks 的派系顺序注册全部炮台。
     * <p>
     * ★ 物品栏顺序 = 方块注册顺序。TestMod 按 PU132 派系顺序调用各分组方法:
     * dark → light → imber → advance (monolith 炮台在 Z_SoulTurrets/Z_AdvTurrets,
     * koruh 经验炮台在 Z_Exp, 由 TestMod 交错编排)。
     */
    ...
    /** dark 派系 7 个炮台 (PU132 UnityBlocks 顺序: apparition, ghost, banshee, fallout, catastrophe, calamity, extinction) */
    public static void loadDark() {
        ...
        apparition = new BoostItemTurret("apparition") {{
            ...
            // ★ 自定义液体强化效果: 水 120%, 冷冻液 145% (末尾0或5的整数百分比)
            ...
            ammo(Items.graphite, new BasicBulletType(7.5f * 1.1f, 50f * 1.4f) {{
                ...
                // standardIncendiaryLarge (火成岩弹, 燃烧型)
                lifetime = 40f;
                width = 16f * 1.12f;
                height = 21f * 1.12f;
                hitSize = 5f;
        ...
        banshee = new BarrelsItemTurret("banshee") {{
            ...
            coolantMultiplier = 0.5f;
            inaccuracy = 3f;
            // ★ PU_V8: spread=37, shots=2, alternate=true
            shoot = new ShootAlternate(37f) {{ shots = 2; }};
        ...
        // ===== fallout (PU_V8 L451-481, BoostLaserTurret + SparkingContinuousLaserBulletType) =====
        fallout = new BoostLaserTurret("fallout") {{
            size = 5;
            health = 3975;
            range = 215f;
            reload = 110f;
        ...
        extinction = new BigLaserTurret("extinction") {{
            ...
            shootType = new SparkingContinuousLaserBulletType(770f) {{
                ...
                lightStroke = 90f;
                width = 18f;  // ★ 激光粗细分级: extinction 最粗 (fallout 8 < calamity 11 < catastrophe 14 < extinction 18)
                fromBlockChance = 0.5f;
                fromBlockDamage = 76f;
                fromLaserChance = 0.8f;
                fromLaserDamage = 46f;
    ...
    /** light 派系 12 个炮台 (PU132 顺序: photon, graviton, electron, proton, neutron, gluon, w-boson, z-boson, higgs-boson, singularity, muon, ephemeron) */
    public static void loadLight() {
        ...
        zBoson = new RampupPowerTurret("z-boson") {{
            ...
            // ★ 用户调整: 射速增长 (ramp) 调慢, speedInc 0.2 → 0.12 (每次装填累计的射速加成减半)
            speedInc = 0.12f;
        ...
        singularity = new PowerTurret("singularity") {{
            ...
            shootType = new BasicBulletType(6.6f, 500f) {  // ★ 修复: BasicBulletType(speed, damage), 原写作 (500f, 7f) 把速度当伤害; 现恢复 PU 原版速度 6.6 并把伤害设为 500
                {
                    lifetime = 110f;
                    drag = 0.018f;
                ...
                public void draw(mindustry.gen.Bullet b) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        float radius = 15f + i * 10f + arc.util.Time.time * 20f;
                        arc.graphics.g2d.Lines.circle(b.x, b.y, radius);
                    }
                    Draw.color();
                }
            };
    ...
    /** imber 派系 7 个炮台 (PU132 顺序: orb, shockwire, current, plasma, electrobomb, shielder, orb-turret) */
    public static void loadImber() {
            ...
            shootType = new BulletType() {
                {
                    ...
                    // ★ 改用带 followParent 的版本, 让光球跟随炮台转动/移动 (见下方 buildType 覆写)
                    chargeEffect = ChargeFx.orbCharge;
                }
        ...
        current = new PowerTurret("current") {{
            ...
            shootType = new LaserBulletType(450f) {{
                ...
                // ★ 改用带 followParent 的版本, 让光球跟随炮台转动/移动 (见下方 buildType 覆写)
                chargeEffect = ChargeFx.currentCharge;
            }};
        ...
        electrobomb = new ItemTurret("electrobomb") {{
            ...
            health = 3650;
            size = 5;
            range = 480f;  // 统一为 60 格显示 (60 × tilesize 8 = 480)
            minRange = 60f;
            reload = 320f;
            shootCone = 20f;
            inaccuracy = 0f;
            ...
            shootEffect = Fx.none;
            smokeEffect = Fx.none;
            // ★ 用户调整: 加金色蓄力特效 (与子弹同色 Pal.surge), 充能 60f 后开火
            shoot.firstShotDelay = 60f;
            ...
            ammo(Z_Items.sparkAlloy, new BasicBulletType(7f, 100f, "large-bomb") {{  // ★ sprite=large-bomb
                ...
                backColor = Pal.surge;
                frontColor = Color.white;
                // ★ 用户调整: 金色发光蓄力特效 (颜色与子弹一致)
                chargeEffect = ChargeFx.electrobombCharge;
        ...
        shielder = new ShieldTurret("shielder") {{
            ...
            shoot.firstShotDelay = 38f;
            // ★ 用户调整: 移除 chargeSound (原在充能开始时响, 听上去像"开火音效在蓄力阶段响");
            //   真正的开火音效由 ShieldTurret.ShieldTurretBuild#bullet() 在实际发射时播放 shootSound。
    ...
    /** advance 派系 6 个炮台 (PU132 顺序: celsius, kelvin, arc-caster, arc-storm, blue-eclipse, xeno-corruptor; the-cube/wavefront 在 Z_AdvTurrets.load3D) */
    public static void loadAdvance() {
        ...
        eclipse = new BoostLaserTurret("blue-eclipse") {{
            ...
            size = 7;
            health = 9000;
            range = 490f;  // ★ 与激光实际长度一致 (shootType.maxLength = 490f)
            reload = 280f;
            shootCone = 40f;
            consumePower(19f);
            shake = 3f;
    ...
    /** absorber 吸收器 (power 类别, PU132 imber 派系发电方块) */
