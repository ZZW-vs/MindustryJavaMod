    ...
    public class WavefrontTurretBuild extends PowerTurretBuild {
        ...
        public void updateTile() {
            ...
            if (isShooting() && canConsume()) {  // v155.4: consValid() → canConsume()
                ...
                // 原 PU_V8: angle += (reload / reloadTime) * objectRotationSpeed;
                // v155.4: reloadTime → reload (Block 字段); reload (Build 计数器) 不变
                angle += (reload / reload) * objectRotationSpeed;
                offset = (reload / reload) * 0.25f;
        ...
        @Override
        public void draw() {
            // 仅在找到 baseRegion 贴图时绘制 (避免显示缺失贴图)
            if (baseRegion.found()) {
            ...
            if (object != null && object.faces != null && object.faces.size > 0) {
                // 应用 scale 到 WavefrontObject 的内部 size 字段
                float originalSize = object.size;
                object.size = scale;

                // 计算旋转: 模型绕 Z 轴旋转 (跟随炮台朝向)
                // 原 PU_V8: inst.transform.set(pos, quaternion(Vec3.Z, rotation - 90f), scale)
                // v155.4 简化: 用 rZ = rotation - 90f, gap 作为轻微 X 轴旋转
                float rZ = rotation - 90f;
                float rX = gap * 30f;  // 间隙效果转为轻微 X 轴倾斜
                float rY = Mathf.sin(angle * Mathf.degRad) * 5f;  // 轻微 Y 轴摆动

                object.draw(x, y, rX, rY, rZ);

                // 恢复 size (避免影响其他用同一 object 的实例)
