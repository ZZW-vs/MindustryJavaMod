...
- **子世界原版化交互**：与原世界的交互体验完全一致，无任何自定义 UI
  ...
  - **点击**：打开原版配置界面 / 物品界面（与原版点击方块完全相同）
  - **建造**：光标落在平台范围时，放置/拆除直接作用于子世界，预览自动吸附子世界网格（连续坐标映射，平台可自由移动和旋转）；建造队列（单点/拖线/蓝图粘贴）落在平台区域的计划自动转译进子世界；建造走原版 ConstructBlock 脚手架（建造动画/资源扣费/事件），完成后新建筑自动注册进子世界正常运行
...
- **渲染升级**（PU132 altBatch 方案）：子世界渲染切换到独立 SpriteBatch + z 排序 —— 建筑内部自由切层（炮台热度发光/电力连线/传送带物品动画等）与原版渲染管线一致；每个建筑绘制黑色左下偏移**阴影**，观感接近原世界；建筑整体显示在单位甲板之上
- **长方形碰撞箱**：与子世界平台同样大小（72x152 像素），随单位旋转
  - 敌方子弹打在整个平台范围被要塞吸收（不再穿过平台）
  - 原版悬停检测覆盖全平台
- **网格对齐修复**：甲板可建造区（8x18）相对单位中心（大地核心在贴图正中）偏左下一格，9x19 子世界多出的一行一列按设计放在**世界左方和最下方**（网格右/顶边界与甲板对齐）；格线相位 0 mod 8 与甲板纹理大格线一致；放置预览公式与原版 `BuildPlan.drawx()` 完全一致（tile*8+offset），ghost 与实际落位零偏差
