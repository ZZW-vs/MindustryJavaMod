...
- **部署模式**：点击 Terra 本体打开控制面板
  - **部署**：子世界建筑落到主世界真实 tile 上，原版输入全功能可用（建造菜单放方块、点击配置、框选、修理、悬停面板），单位锁定不动
  - **部署视觉**：单位自动下沉（elevation=0.5）渲染在所有建筑之下，建筑完整可见；仍保留飞行单位命中判定
  - **收起**：范围内主世界建筑吸回子世界，单位恢复飞行移动
  - **智能让位**：部署后鼠标悬停/点击建筑时模组交互层自动退出，原版建筑提示和配置界面不被遮挡
  - 移动形态下保留悬停/点击交互（点击建筑查看状态和配置）
